...
/// 与硬件,专测 `ReportLiveReadings.readings` 这一层。
@Suite("报表实时读数")
struct ReportLiveReadingsTests {
    ...
    @Test func headlinePassesThroughWithoutAddingPercent() {
        let readings = ReportLiveReadings.readings(for: module(kind: .cpu, summary: "12%"))
    ...
    @Test func headlineKeepsNonPercentSummaryIntact() {
        let readings = ReportLiveReadings.readings(for: module(kind: .battery, summary: "ac-power"))
        #expect(readings?["hwLiveBatteryLevel"] == "ac-power")
    }

    /// 无数据占位不提供该行,让原生右栏显示「—」。
    @Test func placeholderSummaryIsDropped() {
        let readings = ReportLiveReadings.readings(for: module(kind: .gpu, summary: "--"))
    ...
    @Test func networkDownloadComesFromRateMetricNotInterfaceName() {
        let readings = ReportLiveReadings.readings(for: module(
    ...
    @Test func enumIdentifiersAreLocalized() {
        let cpu = ReportLiveReadings.readings(for: module(
