...
<p align="center">
  HagimiMonitor 是一款优雅的 macOS 菜单栏系统监控工具，将 CPU、GPU、内存、存储、网络、电池、蓝牙与显示器状态，以动态圆环和紧凑玻璃面板呈现在菜单栏；并内置数据统计报告与键盘锁定等快捷功能。
</p>
...
- **CPU**：系统 / 用户 / 空闲占用、运行时间、核心分组拆分、热压力，直连版另有温度读取与逐核负载圆环；自适应识别能效 / 性能 / 超核拓扑（M6 及 M5 Pro/Max 三类核芯片自动呈现 S/P/E 三组，其余机型保持 P/E 两组）
- **GPU**：图形负载、渲染 / 分块性能、显存信息、进程级占用
- **内存**：使用率、内存压力、交换分区、压缩内存、压力等级变化
- **存储**：系统磁盘、外部卷容量、读写速率、SMART 健康
- **网络**：上传 / 下载速率、网络接口类型、IP 信息、Wi-Fi 信号与延迟
- **电池与电源**：电量、供电状态、功耗、健康度、循环次数等可用信息
- **蓝牙**：已连接设备电量监控（GATT 优先读取）
- **显示器**：显示器信息双版可看；亮度、音量、对比度控制取决于设备 DDC/CI 支持（仅直连版）

> 温度传感器（CPU 温度）与风扇转速因依赖 SMC（AppleSMC），App Store 沙盒版无法读取，仅在直连版提供。

### 面板布局，随手编排

面板内所有模块行与指标格支持长按拖动排序，拖起时浮起预览、落点实时归位；也可通过右键（触控板双指轻点）呼出菜单，按上下左右逐格移动，并提供模块指标顺序恢复操作。

<p align="center">
  <img src="docs/images/panel-reorder-drag.gif" width="340" alt="长按拖动交换位置">&nbsp;&nbsp;
  <img src="docs/images/panel-reorder-menu.gif" width="340" alt="菜单方向移动换位">
</p>
...
不止动态环形图标——图标、文字、紧凑三种单行横排布局同样可选，总宽静态计算不随数值跳动；还支持将面板钉住窗口、全局快捷键呼出。

### 数据统计与健康评分

HagimiMonitor 会围绕系统状态提供更多长期视角，并自动生成网页可视化报表（默认展示今日，也可切换 24 小时 / 7 天 / 30 天 / 一年等区间）：

- 采样入库，SQLite 分钟 / 小时 / 日分层聚合，自动生成带趋势曲线、热力图的 HTML 报表
- 0-100 分 Mac 健康评分，覆盖 CPU、GPU、磁盘、风扇与内存压力等维度
- 自动检测 CPU 高负载、内存压力、磁盘空间不足、网络流量峰值等关键事件

### 快捷功能

键盘锁定（可拦截包括 F1–F12 在内的全部按键，含 20 分钟自动解锁兜底）、系统防休眠、屏幕不息屏，从菜单栏或钉住面板一键开启，激活状态在面板头部实时点亮。
...
| 数据统计 · 健康评分 · 事件检测（HTML 报表 + 存储管理） | ✅ | ✅ |
| 快捷功能（键盘锁定 / 系统防休眠 / 屏幕不息屏） | ✅ | ✅ |
| 双主题 · 玻璃拟态 · 多种菜单栏样式 | ✅ | ✅ |
| 面板长按拖动排序 · 右键方向移动 | ✅ | ✅ |
| CPU TOP 进程列表 | ✅ | ✅ |
| 内存 TOP 进程列表 | ✅ | ✅ |
| 存储 / 网络 TOP 进程列表 | ✅ | ❌ 沙箱限制 |
| 温度传感器读取（CPU 温度） | ✅ | ❌ SMC 沙箱限制 |
| 风扇转速读取 | ✅ | ❌ SMC 沙箱限制 |
...
> 💚 **App Store 版，是一份心意**：两个版本的核心监控体验一致。如果 HagimiMonitor 帮到了你，而你又不需要显示器控制、存储 / 网络 TOP 进程、温度 / 风扇读数这些沙箱外功能，欢迎在 [App Store](https://apps.apple.com/app/hagimimonitor/id6792169908) 购买。它更像一杯咖啡，是对独立开发者持续维护的一份支持与鼓励。无论你选择哪个版本，都同样感谢。
